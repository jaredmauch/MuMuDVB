...
static struct transponder* find_transponder_by_freq(struct transponder * tn) {
  ...
  if ((tn->type != flags.scantype) || ((tn->frequency < 1) && (!tn->cells->count))) {
     free(buffer);
     ...
     if (flags.scantype == SCAN_TERRESTRIAL) {
        ...
        for(c = (t->cells)->first; c; c = c->next) {
           for(i = 0; i < c->num_center_frequencies; i++) {
              ...
              struct cell* cn;
              int j;
              for(cn = (tn->cells)->first; cn; cn = cn->next) {
                 for(j = 0; j < cn->num_center_frequencies; j++) {
...
static void parse_descriptors(enum table_id t, const unsigned char * buf, int descriptors_loop_len, void *data,
  ...
  while(descriptors_loop_len > 0) {
     ...
     switch(descriptor_tag) {
        case conditional_access_descriptor: /* 20080106 */
                if (t == TABLE_PMT)
  ...
  while(section_length > 0) {
     ...
     switch(buf[0]) { // stream type
        ...
        case iso_iec_13818_1_private_data:
           ...
           else if (find_descriptor(AC_3_descriptor, buf + 5, ES_info_len, NULL, NULL)) {
           ...
           else if (find_descriptor(enhanced_AC_3_descriptor, buf + 5, ES_info_len, NULL, NULL)) {
     ...
     if (t != NULL) {
        ...
        if (flags.add_frequencies > 0 && (tn.type == flags.scantype)) {
           if ((t = find_transponder_by_freq(&tn))) {
              ...
              t = alloc_transponder(tn.frequency, tn.delsys, tn.polarization);
              copy_transponder(t, &tn);
  ...
  while(len > 0) {
     ...
     switch(b[0]) {
        case atsc_service_location_descriptor:
           ...
           break;
        case atsc_extended_channel_descriptor:
  ...
  switch(t->type) {
     case SCAN_SATELLITE:
        ...
        if (scr_config.user_frequency > 0) {
           ...
           // Poll on frontend file descriptor instead of usleep
           struct pollfd pfd = {frontend_fd, POLLIN | POLLPRI, 0};
           int poll_result = poll(&pfd, 1, 100); // 100ms timeout
           if (poll_result < 0 && errno != EINTR) {
             // Poll error, but continue with diseqc sequence
             verbose("Frontend poll error during diseqc: %s\n", strerror(errno));
           }                                
     ...
     if (poll_result < 0 && errno != EINTR) {
       // Poll error, but continue with signal checking
       verbose("Frontend poll error during signal check: %s\n", strerror(errno));
     }
                    }
                 else {
...
static int __tune_to_transponder(int frontend_fd, struct transponder * t, int v) {
  ...
  if (!flags.emulate) {
    ...
    int poll_result = poll(&pfd, 1, 100); // 100ms timeout
    if (poll_result < 0 && errno != EINTR) {
      // Poll error, but continue with tuning
      verbose("Frontend poll error: %s\n", strerror(errno));
    }
  }
 ...
 // look for some signal.
  ...
  while((ret & FE_HAS_LOCK) == 0) {
     ret = check_frontend(frontend_fd,1);
     if (ret != lastret) {
        get_time(&meas_stop);
        verbose(" (%.3fsec): %s%s%s \n",
              elapsed(&meas_start, &meas_stop),
...
static uint16_t check_frontend(int fd, int verbose) {
  ...
  if (verbose) {
     ...
     info("signal %04x (%ddBm) | snr %04x (%ddB) | ber %08x | unc %08x | ", \
                                                  signal, convert_strength_to_dbm(signal), snr, convert_snr_to_db(snr), ber, uncorrected_blocks);
...
static int initial_tune(int frontend_fd, int tuning_data) {
  ...
  char stats_buf[512], *p;
  int stats_buf_len, i, show;
  fe_status_t status;
  int rc;
        struct dvb_device *dvb = NULL;
        struct dvb_dev_list *dvb_dev = NULL;
        struct dvb_v5_fe_parms *parms = NULL;
        struct dvb_v5_fe_parms_priv *parms_priv = NULL;
  ...
  for(delsys_parm = delsys_min; delsys_parm <= delsys_max; delsys_parm++) {
     ...
     if (poll_result < 0 && errno != EINTR) {
       ...
       verbose("Frontend poll error during signal check: %s\n", strerror(errno));
                 ...
                 dvb_dev = dvb_dev_seek_by_adapter(dvb, 1, 0, DVB_DEVICE_FRONTEND);
                 if (!dvb_dev) {

                   perror("!dvb_dev");
                   return -1;
                 }
                 printf("%s:%s\n", dvb_dev->path, dvb_dev->sysname);
                 ...
                 if (0) dvb_fe_prt_parms(parms);

                 stats_buf_len = sizeof(stats_buf);
                 memset(stats_buf, 0, stats_buf_len);
        
                 qual = dvb_fe_retrieve_quality(parms, 0);
                 switch (qual) {
                     case DVB_QUAL_UNKNOWN:
                         printf("\tDVB_QUAL_UNKNOWN\t");
                         break;
                     case DVB_QUAL_POOR:
                         printf("\tDVB_QUAL_POOR\t");
                         break;
                     case DVB_QUAL_OK:
                         printf("\tDVB_QUAL_OK\t");
                         break;
                     case DVB_QUAL_GOOD:
                         printf("\tDVB_QUAL_GOOD\t");
                         break;
                 }

                 parms = dvb->fe_parms;
                 if (dvb->fe_parms->has_v5_stats)
                 {
                     parms_priv = (void *)dvb->fe_parms;
                 }
...
/*                       dvb_fe_snprintf_stat(parms, DTV_STAT_SIGNAL_STRENGTH, ("Signal"),
                                         i, &stats_buf, &stats_buf_len, &show);
     ...
     for(t = new_transponders->first; t; t = t->next) {
        ...
        if (!flags.emulate) {
           // Poll on frontend file descriptor instead of usleep
           struct pollfd pfd = {frontend_fd, POLLIN | POLLPRI, 0};
           int poll_result = poll(&pfd, 1, 1500); // 1500ms timeout
           if (poll_result < 0 && errno != EINTR) {
             verbose("Frontend poll error during initial wait: %s\n", strerror(errno));
           }
        }
        for(cnt=0; cnt<5; cnt++) {
           if (check_frontend(frontend_fd, 1) == 1)
              break;
           // Poll on frontend file descriptor instead of usleep
           struct pollfd pfd = {frontend_fd, POLLIN | POLLPRI, 0};
           int poll_result = poll(&pfd, 1, 200); // 200ms timeout
           if (poll_result < 0 && errno != EINTR) {
             verbose("Frontend poll error during retry: %s\n", strerror(errno));
           }
           }
